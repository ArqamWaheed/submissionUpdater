...
async function main() {
  ...
  console.log('\nComparison report (FETCHED vs VALID):')
  console.log(`Compared at: ${report.comparedAt}`)
  console.log(`FETCHED = data/courses.json  (source: website)`)
  console.log(`VALID   = data/courses_from_sheet.json  (authoritative sheet)`)
  console.log(`Total semesters compared: ${report.semesters.length}`)
  console.log(`Total differences found: ${totalDiffs}\n`)

  for (const sem of report.semesters) {
    // Human-friendly English summary header
    console.log(`\n${sem.name}: ${sem.diffCount} difference(s) found between data from website and authoritative sheet.`)
    for (const d of sem.diffs) {
      if (d.type === 'missing-in-fetched') {
        const c = d.course || {}
        console.log(`In ${sem.name} of the authoritative sheet, the website is missing ${d.count} copy(ies) of:`)
        console.log(`  code='${c.code || ''}', title='${c.title || ''}', credits='${c.credits || ''}'`)
        continue
      }
      if (d.type === 'count-mismatch') {
        const c = d.course || {}
        console.log(`In ${sem.name}, counts differ for course code='${c.code || ''}', title='${c.title || ''}': website has ${d.fetchedCount}, sheet has ${d.validCount}.`)
        continue
      }
      if (d.type === 'code-mismatch') {
        const f = d.fetched || {}
        const v = d.valid || {}
        console.log(`In ${sem.name}, a course appears with a different code in the website vs sheet: website code='${f.code || ''}'  authoritative code='${v.code || ''}', credits='${normalizeCredits(f.credits) || normalizeCredits(v.credits)}'.`)
        continue
      }
      if (d.type === 'extra-in-fetched') {
        const c = d.course || {}
        console.log(`In ${sem.name}, the website has ${d.count} extra copy(ies) of a course not present in the authoritative sheet:`)
        console.log(`  code='${c.code || ''}', title='${c.title || ''}', credits='${c.credits || ''}'`)
        continue
      }
    }
